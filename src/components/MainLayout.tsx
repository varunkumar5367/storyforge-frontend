...
export default function MainLayout({ children }: MainLayoutProps) {
  ...
  const checkServerStatus = useCallback(async () => {
    try {
      ...
      if (data.status === 'online' && data.tunnel_url) {
        try {
          ...
          if (healthRes.ok) {
            ...
            localStorage.setItem('storyforge_server_offline', 'false');
      ...
      localStorage.setItem('storyforge_server_offline', 'true');
      window.dispatchEvent(new CustomEvent('server-status-changed'));
      return false;
    } catch (error) {
  ...
  // 2. Check server status first (covers only /studio page if offline)
  const isStudioPage = pathname.startsWith('/studio');
  if (serverState !== 'online' && isStudioPage) {
    return (
      <div style={{
        display: 'flex',
        alignItems: 'center',
      ...
      <aside className={`${styles.sidebar} ${sidebarCollapsed ? styles.sidebarCollapsed : ''}`}>
        ...
        <nav className={styles.navSection}>
          ...
          {role === 'admin' && (
            <Link 
              href="/accuracy" 
              className={`${styles.navLink} ${pathname === '/accuracy' ? styles.navLinkActive : ''}`}
            ...
            >
              ...
              {!sidebarCollapsed && <span>Accuracy Report</span>}
            </Link>
          )}
