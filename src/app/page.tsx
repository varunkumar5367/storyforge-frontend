  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.welcomeHeader}>
        <h2 className={styles.welcomeTitle}>Welcome back, Video Creator</h2>
        <p className={styles.welcomeDesc}>Transform scripts into cinematic stories. View your generation queue and account metrics below.</p>
        {jobs.length > 0 && (
          <Link href="/studio" className={styles.createBtnCentred}>
      ...
      <div className={styles.statsGrid}>
        ...
        <div className={`glass ${styles.statCard}`}>
          ...
          <div className={styles.statContent}>
            ...
            <span className={styles.statSub}>Ready for download</span>
          </div>
        </div>

        {/* Metric 2 */}
        <div className={`glass ${styles.statCard}`}>
          <div className={styles.statIcon} style={{ background: 'rgba(6, 182, 212, 0.15)', color: 'var(--accent-cyan)', borderColor: 'rgba(6, 182, 212, 0.2)' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="22 12 18 12 15 21 9 3 6 12 2 12" />
            </svg>
          </div>
          <div className={styles.statContent}>
            <span className={styles.statTitle}>Processing / Queued</span>
            <span className={styles.statValue}>{runningJobs.length + jobs.filter(j => j.status === 'pending').length}</span>
            <span className={styles.statSub}>In compilation pipeline</span>
          </div>
        </div>

        {/* Metric 3 */}
        <div className={`glass ${styles.statCard}`}>
          <div className={styles.statIcon} style={{ background: 'rgba(16, 185, 129, 0.15)', color: 'var(--accent-green)', borderColor: 'rgba(16, 185, 129, 0.2)' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div className={styles.thumbnailArea}>
                      {thumbnailUrl ? (
                        ...
                        <img 
                          ...
                          onError={(e) => {
                            // If image fails, replace with icon placeholder
                      ...
                      {(!thumbnailUrl) && (
                        <div className={styles.thumbnailFallback}>
                          ...
                          <span style={{ fontSize: '0.75rem', fontWeight: 600 }}>
                            {isProcessing ? 'Generating Asset...' : 'Generation Failed'}
                          </span>
                        </div>
                      )}
            ...
            {jobs.length > 3 && (
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: '32px' }}>
                <Link href="/history" className={styles.createBtn} style={{ margin: 0, textDecoration: 'none' }}>
                  📂 Search & Manage All {jobs.length} Productions
                </Link>
              </div>
            )}
          </div>
